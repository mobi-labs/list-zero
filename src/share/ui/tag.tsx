...
import { TagType } from '@/entities/model'
import { cn } from '@/share/libs'

const TagVariants = cva(
  'font-pretendard leading-[120%] text-[12px] inline-flex justify-center items-center px-[6px] py-[3px]',
  {
    variants: {
      level: {
...
export function Tag({ name, rectangle, level, ...props }: Props) {
  return (
    <div className={cn(TagVariants({ rectangle, level }))} {...props}>
      {name}
    </div>
  )
}
